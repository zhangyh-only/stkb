    ...
    <section class="build-shell">
      ...
      <section v-else-if="activeView === 'rules'" class="rules-library-view">
        <div v-if="catalog" class="rules-library-grid">
          ...
          <article v-if="selectedDomain && selectedModule" class="rules-detail"><header><div><p>{{ selectedModule.code }}</p><h2>{{ selectedModule.name }}</h2></div><span>{{ catalog.version }}</span></header><div class="rules-detail-scroll"><section class="domain-definition"><b>{{ selectedDomain.question }}</b><strong>{{ selectedDomain.meaning }}</strong><p>{{ selectedDomain.boundary }}</p></section><section><h3>模块定义</h3><p>{{ selectedModule.meaning }}</p></section><section class="rule-highlight"><h3>识别与对象边界</h3><p>{{ selectedModule.boundary }}</p></section><section><h3>允许形成的对象类型</h3><div class="rule-type-list"><code v-for="item in selectedModule.objectTypes" :key="item">{{ item }}</code></div></section><section><h3>对象表达</h3><p>{{ selectedModule.coreObjects.join('、') }}</p></section><section><h3>适用资料</h3><p>{{ selectedModule.sources.join('、') }}</p></section><section><h3>使用方</h3><p>{{ selectedModule.consumers.join('、') }}</p></section></div></article>
        </div>
      </section>

      <section v-else class="run-evidence-view">
        <div v-if="identification" class="evidence-layout">
          <aside class="call-list"><div><p>MODEL EXCHANGES</p><h2>模型交互</h2></div><button v-for="(call, index) in identification.modelCalls" :key="`${call.attempt}-${call.purpose}`" type="button" :class="{ active: selectedCallIndex === index }" @click="selectedCallIndex = index"><span>{{ index + 1 }}</span><div><strong>{{ call.purpose === 'identification' ? '知识识别' : call.purpose === 'repair' ? '结构修复' : '输出续接' }}</strong><small>{{ call.segment || '全文' }} · {{ formatDuration(call.durationMs) }}</small></div></button></aside>
