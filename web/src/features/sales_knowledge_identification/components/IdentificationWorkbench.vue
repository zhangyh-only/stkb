    ...
    <section class="build-shell">
      ...
      <section v-else class="run-evidence-view">
        <div v-if="identification" class="evidence-layout">
          ...
          <aside class="run-summary"><section><span>规则版本</span><strong>{{ identification.catalogVersion }}</strong></section><section><span>提示词版本</span><strong>{{ identification.promptVersion }}</strong></section><section><span>输出合同</span><strong>{{ identification.schemaVersion }}</strong></section><section><span>耗时</span><strong>{{ formatDuration(identification.durationMs) }}</strong></section><section><span>识别结果</span><strong>{{ identification.candidates.length }}项候选 / {{ identification.rejectedCandidates.length }}项拒绝</strong></section></aside>
        </div>
