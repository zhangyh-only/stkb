  ...
  <main class="knowledge-build-app">
    ...
    <section class="build-shell">
      ...
      <section v-else-if="activeView === 'knowledge'" class="knowledge-result-view">
        <div v-if="formation?.releaseBlockers.length" class="release-blockers"><strong>本次结果未发布到正式知识</strong><span v-for="item in formation.releaseBlockers" :key="item">{{ item }}</span></div>
        <div v-if="storageEvidence" class="knowledge-levels"><span>本次发布KnowledgeObject <b>{{ storageEvidence.postgresObjects }}</b></span><span>内部item <b>{{ storageEvidence.internalItems }}</b></span><span>RetrievalUnit <b>{{ storageEvidence.pgvectorRecords }}</b></span></div>
      ...
      <section v-else-if="activeView === 'rules'" class="rules-library-view">
        <div v-if="selectedModule" class="knowledge-levels"><span>顶层KnowledgeObject合同 <b>{{ selectedModule.objectTypes.length }}</b></span><span>内部item类型 <b>{{ selectedModule.itemTypes.length }}</b></span></div>
