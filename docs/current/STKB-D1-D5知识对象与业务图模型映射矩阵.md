...
D2 管理可复用的客户特征、需求、动因、触发机制和决策角色。只有能够跨客户或销售场景复用的客户认知才进入 D2；单个客户事实、一次沟通表现和未经验证的心理推断不能直接泛化为画像或反应规律。
...
| D2.2 画像库 | 管理由稳定特征、关注重点、决策方式和行为边界共同构成的客户画像，以及支撑画像判断的背景事实 | 客户画像、画像背景事实 | 行业、渠道、销售场景 | 画像是可复用的客户类型定义，不是人口属性标签的堆叠；单个客户实例、一次沟通态度和仅由销售经验推测的反应不能直接写入画像；有明确触发条件的稳定反应规律归 D2.4 | 画像研究、客户访谈、业务经验材料、典型案例 | D2 画像识别、特征归一、画像组合与归并 | 客户识别、场景装配、策略检索、个性化话术 |
...
D3 管理可复用的销售方法、策略判断、场景阶段和合规规则。某次销售执行的当前阶段、已采取动作和临时下一步属于运行记录，不作为策略知识。
